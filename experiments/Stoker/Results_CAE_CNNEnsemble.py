...
def results(runName, minValidEpoch):
    ...
    var = np.zeros_like(pred)#predData['var'][0]
    ...
    savePath = join(experPaths.run, f'CAE_CNNEns_Stok_Error_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'CAE_CNNEns_Stok_predPlot{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
    plotData = {'pred': pred, 'target': target}
    savePath = join(experPaths.run, f'CAE_CNNEns_Stok_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [50, 100, 150]:
        savePath = join(experPaths.run, f'CAE_CNNEns_Stok_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
        plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
