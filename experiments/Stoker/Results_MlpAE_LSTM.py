...
def results(runName, minValidEpoch):
    ...
    savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_imgPred{0}_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_imgTar{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest, 'v_min':hp.dataMin, 'v_max':hp.dataMax}
    ...
    savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_Error_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_predPlot{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
    plotData = {'pred': pred, 'target': target}
    savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [50, 100, 150]:
        savePath = join(experPaths.run, f'MlpAE_LSTM_Stok_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
        plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
...
name = 'results_MlpAE_LSTM_ld50_sql10_layers1_lr0.0003_trSmp250_bs15M8ZMT'
