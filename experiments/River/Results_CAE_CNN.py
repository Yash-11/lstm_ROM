...
def results(runName, minValidEpoch):
    ...
    # savePath = join(experPaths.run, f'CAE_CNN_River_imgPred{0}_epoch{hp.loadWeightsEpoch}')
    ...
    # savePath = join(experPaths.run, f'CAE_CNN_River_imgTar{0}_epoch{hp.loadWeightsEpoch}')
    # plotParams = {'tStepModelPlot':[2]*hp.numSampTest, 'v_min':hp.dataMin, 'v_max':hp.dataMax}
    # plotData = {'data': target}
    # Plots().imgPlot(plotData, Dict2Class(plotParams), savePath)

    # savePath = join(experPaths.run, f'CAE_CNN_River_cbar{0}_epoch{hp.loadWeightsEpoch}')
    # Plots().cbar({}, Dict2Class(plotParams), savePath)

    # savePath = join(experPaths.run, f'CAE_CNN_River_img{0}_epoch{hp.loadWeightsEpoch}')
    # plotParams = {'tStepModelPlot':[2]*hp.numSampTest, 'v_min':hp.dataMin, 'v_max':hp.dataMax}
    ...
    savePath = join(experPaths.run, f'CAE_CNN_River_Error_epoch{hp.loadWeightsEpoch}')
    ...
    # savePath = join(experPaths.run, f'CAE_CNN_River_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [10, 30, 60]:
      savePath = join(experPaths.run, f'CAE_CNN_River_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
      plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
...
name = 'results_CAE_CNN_ld4_sql20_krs3_lr0.0003_trSmp50_ch100100100_bs16_8PA67' 
