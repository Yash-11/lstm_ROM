...
def results(runName, minValidEpoch):
    ...
    savePath = join(experPaths.run, f'CAE_LSTM_BurRe{hp.Re}_imgPred{0}_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'CAE_LSTM_BurRe{hp.Re}_imgTar{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest, 'v_min':hp.dataMin, 'v_max':hp.dataMax}
    ...
    savePath = join(experPaths.run, f'CAE_LSTM_Bur_Error_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'CAE_LSTM_Bur_predPlot{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
    plotData = {'pred': pred, 'target': target}
    savePath = join(experPaths.run, f'CAE_LSTM_Bur_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [20, 40, 60]:
      savePath = join(experPaths.run, f'CAE_LSTM_Bur_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
      plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
...
name = 'results_CAE_LSTM_Re600_sql10_lr0.0005_bs15_AE7_FVXMG'
