...
def results(runName, minValidEpoch):
    ...
    savePath = join(experPaths.run, f'CAE_TCN_Bur_predPlot{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
    plotData = {'pred': pred, 'target': target}
    savePath = join(experPaths.run, f'CAE_TCN_Bur_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [20, 40, 60]:
      savePath = join(experPaths.run, f'CAE_TCN_Bur_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
      plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
