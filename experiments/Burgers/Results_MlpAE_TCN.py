...
def results(runName, minValidEpoch):
    ...
    savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_imgPred{0}_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_imgTar{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest, 'v_min':hp.dataMin, 'v_max':hp.dataMax}
    ...
    savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_Error_epoch{hp.loadWeightsEpoch}')
    ...
    savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_predPlot{0}_epoch{hp.loadWeightsEpoch}')
    plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
    plotData = {'pred': pred, 'target': target}
    savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_predimPlot{0}_epoch{hp.loadWeightsEpoch}')
    ...
    for timestepplot in [20, 40, 60]:
        savePath = join(experPaths.run, f'MlpAE_TCN_BurRe{hp.Re}_predgraphPlot{timestepplot}_epoch{hp.loadWeightsEpoch}')
        plotParams = {'tStepModelPlot':[2]*hp.numSampTest}
...
name = 'results_MlpAE_TCN_Re600_ld10_sql10_krs3_lr0.0003_ch100100_bs15_FVXMG'
