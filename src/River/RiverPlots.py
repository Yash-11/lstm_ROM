...
class Plots:
    ...
    def plotPredSingle(self, plotData: Dict, plotParams, savePath: str, timestep):
        ...
        ax.plot(np.diagonal(target[0]), label = 'training end')
    ...
    def plotPredSingleAE(self, plotData: Dict, plotParams, savePath: str, timestep):
        ...
        # ax.plot(np.fliplr(target[timestep]).diagonal(), label = 'ground truth')
        # ax.plot(np.fliplr(pred[timestep]).diagonal(), label = 'prediction')
